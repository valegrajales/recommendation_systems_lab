...
public class CrossValidationRunner {
	 ...
	 */
	public static boolean useDifferentialSplitCreation = true;
		...
		for (int vround = 0; vround < xvalidationRounds; vround++) {
			...
			if (dataSplitter.getSpecialTestSplits() == null) {
				...
				for (int i = 0; i < dataSplits.size(); i++) {
					if (i != vround) {
//						System.out.println("Setting training data: " + dataSplits.get(i).size() + " (" + i + ")");
            ...
            // Does not work with implicit ratings
            if (useDifferentialSplitCreation) {
            	...
            	// If there's additional data, let's store the original information
            	// and add the ratings to the training model
            	if (dataSplitter.getAdditionalTrainingData().size() > 0) {
            		...
            		// Copy the stuff for later use
            		for (Rating r : trainingData) {
            			Set<Rating> origRatingsOfUser = trainingDM.originalTrainingPerUser.get(r.user);
            			if (origRatingsOfUser == null) {
            				origRatingsOfUser = new HashSet<Rating>();
            				trainingDM.originalTrainingPerUser.put(r.user, origRatingsOfUser);
            			}
            			origRatingsOfUser.add(r);
            		}
            	...
            	System.out.println("Training " + trainingDM);
            	System.out.println("Test " + testDM);
